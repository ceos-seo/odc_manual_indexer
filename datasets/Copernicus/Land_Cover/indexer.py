...
def make_metadata_doc(path, product_name):
    ...
    start_time_str = data.attrs['time_coverage_start']
    start_time = datetime.datetime.strptime(start_time_str, '%Y-%m-%dT%H:%M:%SZ')
    end_time_str = data.attrs['time_coverage_end']
    end_time = datetime.datetime.strptime(end_time_str, '%Y-%m-%dT%H:%M:%SZ')
    center_dt = start_time + ((end_time - start_time) / 2)
    creation_dt_str = data.attrs['file_creation']
    creation_dt = datetime.datetime.strptime(creation_dt_str, '%a %b %d %H:%M:%S %Y')

    spatial_ref = osr.SpatialReference()
    spatial_ref.ImportFromEPSG(4326)
    geo_ref_points = get_geo_ref_points(tif_paths[0])
    print(f'Geo Ref Points: {geo_ref_points}')
    ...
    for band_file_name, band_name in band_map.items():
        for tif_path in tif_paths:
            match = re.search(band_file_name, tif_path)
            if match:
                doc_bands[band_name] = {'path': tif_path}
                break

    doc = {
        'id': str(uuid.uuid5(uuid.NAMESPACE_URL, path)),
        'product': {'name': product_name},
        'instrument': {'name': 'VEGETATION'},
        'product_type': 'CGLS-LC100',
        'creation_dt': creation_dt,
        'platform': {'code': 'Proba-V'},
        'extent': {
            'from_dt': start_time,
            'to_dt': end_time,
            'center_dt': center_dt,
...
def worker(config, path, product_name, unsafe, queue):
    ...
    while True:
        try:
            ...
            if data:
                uri = 'file:/' + data['image']['bands']['Discrete_Classification_map']['path']
